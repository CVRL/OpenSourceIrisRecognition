...
if ! test -d "$ROOT/libtorch/"; then
  wget https://download.pytorch.org/libtorch/cpu/libtorch-cxx11-abi-static-with-deps-2.1.2%2Bcpu.zip
  unzip libtorch-cxx11-abi-static-with-deps-2.1.2+cpu.zip
  rm libtorch-cxx11-abi-static-with-deps-2.1.2+cpu.zip
fi
...
cp "$ROOT/build/libfrvt_1N_nd_cvrl_tripletnn_${FRVT_VER}.so" "$ROOT/frvt/1N/lib/"
